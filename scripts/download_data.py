...
def check_kaggle_credentials() -> bool:
    ...
    if not kaggle_json.exists():
        logger.error("Kaggle API credentials not found at %s. Please create an API token from your Kaggle account and place it there.", kaggle_json)
        return False
    return True
...
def download_dataset(output_dir: Path) -> None:
    ...
    try:
        subprocess.run(["kaggle", "datasets", "download", "-d", KAGGLE_DATASET, "-p", str(output_dir), "--unzip"], check=True)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("data/raw"), help="Directory to save the raw dataset")
    args = parser.parse_args()

    if not check_kaggle_credentials():
        logger.warning(
            "Without Kaggle credentials, you must manually download the dataset from https://www.kaggle.com/datasets/%s and extract it to %s",
            KAGGLE_DATASET,
            args.output_dir,
        )
        sys.exit(1)
